...
helpFunction()
{
   ...
   echo -e "\t-k Kubernetes namespace (default = jkapp)"
...
cleanupFunction()
{
   ...
   $HELMDIR/helm --namespace $BIKENS uninstall sample-nodejs
...
if [ ! -d "$REPOROOT/samples/sample-nodejs" ]; then
  echo "$(tput setaf 1)ERROR: sample-nodejs not found in $REPOROOT/samples/sample-nodejs$(tput sgr 0)"
  echo "Did you download the git repo?"
  echo "Run; git clone https://github.com/jyothibasuk/sample-nodejs.git [local-repo-path]"
  exit 1
fi
...
CHARTDIR="$REPOROOT/samples/sample-nodejs/charts/"
echo "---"
echo "Chart directory: $CHARTDIR"



echo "helm install sample-nodejs (average time to install = 4 minutes)"
$HELMDIR/helm install sample-nodejs "$CHARTDIR" \
